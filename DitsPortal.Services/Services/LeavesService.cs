...
namespace DitsPortal.Services.Services
{
    public class LeavesService : ILeavesService
    {
        ...
        public async Task<BaseResponse> CreateLeave(LeaveRequest leaveRequest, SmtpRequest smtpRequest,string admin)
        {
            try
            {
                //var getPendingId = _globalCodesRepository.GetSingle(leaveRequest).CodeName;
                ...
                //CreateLeave.PendingLeaves  = leaveRequest.PendingLeaves ;
                //CreateLeave.RequestStatus = pending;
        ...
        public async Task<BaseResponse> GetAllLeaveRecordofEmployees(EmployeeLeaveRequest employeeleaveRequest)
        {
            try
            {
                var leaveResponse = await _leavesRepository.GetAllLeaveRecordofEmployees(employeeleaveRequest);
                if (leaveResponse.totalRecords != 0)
                {
