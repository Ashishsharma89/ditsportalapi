...
namespace DitsPortal.Common.StaticResources
{
    public static class NotificationHelper
    {
        ...
        public static bool SendPasswordResetEmail(string emailAddress, string guid, SmtpRequest smtpRequest)
        {
            try
            {
                ...
                emailMessage.Append("<p>You have requested a password reset for the DITS website.</p>");
                emailMessage.Append(string.Format("<p><a href='{0}?email={1}&token={2}'>Please click here to change your password</a></p>", "http://ditstekdemo.com/ditsportal/change-password", emailAddress, guid));
                emailMessage.Append("<p>If you did not request a password reset, please just ignore this email.");
                emailMessage.Append("<p>Thank you for using Dispatch Tracker!</p>");

                SendEmail(emailAddress, emailMessage, "DITS - Password Reset Request", true, smtpRequest);
        ...
        public static bool SendLeaveRequestEmail(LeaveResponse leaveResponse,SmtpRequest smtpRequest,string admin)
        {
            ...
            try
            {
                ...
                //emailMessage.Append("<p>Start Date - " + leaveRequest.StartDate + "</p>");
                //emailMessage.Append("<p>End Date - " + leaveRequest.EndDate + "</p>");
                //emailMessage.Append("<p>Leave Type - " + leaveRequest.LeaveType + " </p>");
                //emailMessage.Append("<p>Duration - " + leaveRequest.Duration + "</p>");
                //emailMessage.Append("<p>Comments - " + leaveRequest.Comment + "</p>");
                emailMessage.Append("<p>Kindly consider for your kind approval. </p>");
                emailMessage.Append("<p>Thanks </p>");
                emailMessage.Append("<p>" + leaveResponse.Email + "</p>");
