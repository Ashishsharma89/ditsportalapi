...
namespace DitsPortal.DataAccess.Repositories
{
...

    {
        ...
        public async Task<LeaveMainResponse> GetAllLeaveRecordofEmployees(EmployeeLeaveRequest employeeleaveRequest)
        {
            IQueryable<LeaveResponse> data;
        ...
        async Task<BaseResponse> ILeavesRepository.ApproveRejectLeave(List<ApproveDenyLeave> approveDenyLeaveRequest, SmtpRequest smtpRequest, string admin)
        {
            _baseResponse = new BaseResponse();
                ...
                if (approveDenyLeaveRequest.Count > 0)
                {
                    foreach (var leave in approveDenyLeaveRequest)
                    {
                        ...
                        if (leave.IsApproved == true)
                        {
                            ...
                            //update leaves table
                            leaves.RequestStatus = 8;
                        }
                        else
                        {
                            leaves.RequestStatus = 9;
                            leaves.RejectedBy = "Admin";
                            leaves.RejectionDate = DateTime.Now;
                            leaves.RejectionReason = leave.RejectionReason;
                        }
                        ...
                        leaves.ModifiedOn = DateTime.Now;
                        await ObjContext.SaveChangesAsync();
                        string email = await GetUserById(leave.EmployeeId);
                        bool sendEmail = NotificationHelper.SendLeaveStatusEmail(email, smtpRequest, leave.IsApproved,admin);
                        _baseResponse.Status = true;
                        _baseResponse.Message = Constants.LEAVE_APPROVED;
